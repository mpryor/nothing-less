...
| **Column delimiter** | A secondary delimiter applied to a single column to split it into sub-columns (`d`). Supports `json`, `kv` (key=value pairs), regex, and string delimiters. A naming wizard prompts you to rename each new column after splitting. Columns that look splittable show a `⑃` label in the header. |
